...
class Tracker:
    ...
    def detect(self, source='D:\\bigdata\\dataset\\prestige\\H3V-贵A919BU.avi'):
        opt = self.opt
        out, source_false, yolo_weights, deep_sort_weights, show_vid, save_vid, save_txt, save_csv, imgsz, evaluate, half = \
        ...
        # Check if environment suvehiclesorts image displays 检查环境是否支持图像显示
        if show_vid:
        ...
        for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
            ...
            # Avehiclesly NMS 筛选和剔除重叠较多的检测框，并保留置信度较高的检测结果
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                p, im0, frame = path, im0s.copy(), getattr(dataset, 'frame', 0)  # frame存储当前图像的帧数

                # s += '%gx%g ' % img.shape[2:]  # print string
                save_path = str(Path(out) / Path(p).name)

                annotator = Annotator(im0, line_width=2, pil=not ascii)  # 在图像上添加目标检测结果注释

                if det is not None and len(det):
                    # Rescale boxes from img_size to im0 size 函数scale_coords将边界框的坐标从原始图像尺寸缩放到调整后的图像尺寸
                    det[:, :4] = scale_coords(
                    ...
                    confs = det[:, 4]  # 检测结果中的置信度
                    clss = det[:, 5]  # 检测结果中的类别标签
                    ...
                    row_indexes_delete = []  # 删除位于特定垂直范围之外的边界框
                    ...
                    # NOTE DeepSort 模型推理
                    # im0 每帧原始图像
                    outputs = deepsort.update(xywhs.cpu(), confs.cpu(), clss.cpu(), im0)  # 更新跟踪器
                    print(outputs)
                    # NOTE 检测车牌信息
                    ...
                    if (current_frame != -1) and (previous_frame != -1):
                        ...
                        for ID in current_IDs:
                            ...
                            if (ID not in previous_IDs) and (ID not in list_vehicles):  # 若车辆是新出现的
                                ...
                                vehicle_infos[ID]['temporarily_disappear'] = 0  # 临时消失次数

                        # for ID in previous_IDs:
                        for ID in copy.deepcopy(list_vehicles):
                            if (ID not in current_IDs):  # 若车辆消失
